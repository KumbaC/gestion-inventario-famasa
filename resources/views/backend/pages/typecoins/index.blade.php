                                    ...
                                    <td class="border px-4 py-2">
                                        <div class="flex gap-2 justify-center">
                                            ...
                                            {{-- @if (auth()->user()->can('typecoin.edit')) --}}
                                                <a href="{{ route('admin.type-coins.edit', $typeCoin->id) }}" class="text-blue-600 hover:underline">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                            {{-- @endif --}}
                                           {{--  @if (auth()->user()->can('typecoin.delete')) --}}
                                                <form action="{{ route('admin.type-coins.destroy', $typeCoin->id) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure you want to delete this item?') }}');">
                                                    @csrf
                                                    @method('DELETE')
