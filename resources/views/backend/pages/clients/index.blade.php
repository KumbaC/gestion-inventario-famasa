    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="px-5 py-4 sm:px-6 sm:py-5 flex justify-between items-center border-b border-gray-100 dark:border-gray-800">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90">{{ __('Clientes') }}</h3>
                ...
                <table class="table-auto w-full border-collapse rounded-2xl">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-2">{{ __('Identificación') }}</th>
                            <th class="px-4 py-2">{{ __('Teléfono') }}</th>
                    ...
                    <tbody>
                            @foreach ($clients as $client)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">{{ $client->identification }}</td>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <p class="text-gray-600 dark:text-gray-400 mt-2">
                                                                {{ __('¿Estás seguro de que deseas editar este cliente?') }}
                                                            </p>
                                            ...
                                            @if (auth()->user()->can('client.delete'))
                                                <div x-data="{ deleteModalOpen: false }">
                                                    ...
                                                    <x-modals.confirm-delete
                                                        id="delete-modal-{{ $client->id }}"
                                                        title="{{ __('Eliminar Cliente') }}"
                                                        content="{{ __('¿Estás seguro de que deseas eliminar este cliente?') }}"
                                                        formId="delete-form-{{ $client->id }}"
                                                        formAction="{{ route('admin.clients.destroy', $client->id) }}"
