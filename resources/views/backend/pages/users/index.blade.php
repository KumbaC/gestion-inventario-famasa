    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
          <div class="px-5 py-4 sm:px-6 sm:py-5 flex gap-3 md:gap-1 flex-col md:flex-row justify-between items-center">
                <h3 class="text-base font-medium text-gray-800 dark:text-white/90 hidden md:block">{{ __('Usuarios') }}</h3>

                @include('backend.partials.search-form', [
                    'placeholder' => __('Buscar por nombre o correo electrónico'),
                ])
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center justify-center" x-show="selectedUsers.length > 0">
                        <button id="bulkActionsButton" data-dropdown-toggle="bulkActionsDropdown" class="btn-danger flex items-center justify-center gap-2 text-sm" type="button">
                            ...
                            <span>{{ __('Acciones masivas') }} (<span x-text="selectedUsers.length"></span>)</span>
                            <i class="bi bi-chevron-down"></i>
                        </button>

                        <!-- Bulk Actions dropdown menu -->
                        <div id="bulkActionsDropdown" class="z-10 hidden w-48 p-3 bg-white rounded-lg shadow dark:bg-gray-700">
                            <h6 class="mb-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('Acciones masivas') }}</h6>
                            <ul class="space-y-2">
                                <li class="cursor-pointer text-sm text-red-600 dark:text-red-400 hover:bg-gray-200 dark:hover:bg-gray-600 px-2 py-1 rounded"
                                    ...
                                    <i class="bi bi-trash mr-1"></i> {{ __('Eliminar seleccionados') }}
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <table id="dataTable" class="w-full dark:text-gray-400">
                    <thead class="bg-light text-capitalize">
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th width="15%" class="p-2 bg-gray-50 dark:bg-gray-800 dark:text-white text-left px-5">{{ __('Acción') }}</th>
                                        ...
                                        @if (auth()->user()->canBeModified($user, 'user.delete'))
                                            <div x-data="{ deleteModalOpen: false }">
                                                ...
                                                <x-modals.confirm-delete
                                                    ...
                                                    content="{{ __('¿Está seguro de que desea eliminar este usuario?') }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    <p class="text-gray-500 dark:text-gray-400">{{ __('No se encontraron usuarios') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        >
            <div class="flex items-center justify-between border-b border-gray-100 px-4 py-2 dark:border-gray-800">
                ...
                <h3 id="bulk-delete-modal-title" class="font-semibold tracking-wide text-gray-800 dark:text-white">
                    {{ __('Eliminar usuarios seleccionados') }}
                </h3>
                <button 
            ...
            <div class="px-4 text-center">
                <p class="text-gray-500 dark:text-gray-400">
                    {{ __('¿Está seguro de que desea eliminar los usuarios seleccionados?') }} 
