...
<div>
    <div class="mb-5 sm:mb-8">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400">
        {{ __('¡Introduce tu email y contraseña para iniciar sesión!') }}
      </p>
    </div>
    <div>
        ...
        <div class="space-y-5">
          ...
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ __('Correo Electronico') }} <span class="text-error-500">*</span>
            </label>
          ...
          <div>
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
              {{ __('Contraseña') }} <span class="text-error-500">*</span>
            </label>
            <div x-data="{ showPassword: false }" class="relative">
              <input :type="showPassword ? 'text' : 'password'" autocomplete="current-password" name="password" placeholder="{{ __('Escriba su contraseña') }}" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pl-4 pr-11 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800" type="password">
          ...
          <div class="flex items-center justify-between">
            ...
            {{-- <a href="{{ route('admin.password.request') }}" class="text-sm text-brand-500 hover:text-brand-600 dark:text-brand-400">{{ __('¿Olvidaste tu contraseña?') }}</a> --}}
          </div>
          <!-- Button -->
          <div>
